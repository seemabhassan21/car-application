...
from app.api.cars.car_schema import CarCreate, CarUpdate, CarResponse, CarListResponse
import uuid
...
async def create_car(car_data: CarCreate, session: AsyncSession = Depends(get_db)):
    repo = CarRepository(session)
    ...
    return CarResponse.from_repo(car_record)


@router.get("/", response_model=CarListResponse)
async def list_cars(limit: int = 10, session: AsyncSession = Depends(get_db)):
    repo = CarRepository(session)
    car_records = await repo.list_cars(limit)
    return CarListResponse.from_repo_list(car_records)


@router.get("/{car_id}", response_model=CarResponse)
async def get_car(car_id: str, session: AsyncSession = Depends(get_db)):
    car_record = await fetch_car_or_404(car_id, session)
    return CarResponse.from_repo(car_record)


@router.put("/{car_id}", response_model=CarResponse)
async def replace_car(
    car_id: str, car_data: CarCreate, session: AsyncSession = Depends(get_db)
):
    repo = CarRepository(session)
...
async def update_car(
    car_id: str, update_data: CarUpdate, session: AsyncSession = Depends(get_db)
):
    repo = CarRepository(session)
...
async def delete_car(car_id: str, session: AsyncSession = Depends(get_db)):
    repo = CarRepository(session)
